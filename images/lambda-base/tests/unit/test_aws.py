...
def test_session_kwargs_include_endpoint_only_when_set():
    ...
    assert aws.session_kwargs(Settings(aws_region="eu-west-1", aws_endpoint_url="http://localhost:4566")) == {
...
def test_resources_are_cached_per_settings():
    ...
    assert aws.dynamodb_resource(local) is aws.dynamodb_resource(Settings(aws_endpoint_url="http://localhost:4566"))
