...
def _all_stacks_exist(fake: FakeAws, catalog: Catalog) -> None:
...
def test_empty_account_creates_everything(catalog: Catalog, fake: FakeAws) -> None:
    plan = build_plan(catalog, fake.aws)
    actions = _by_target(plan)
    assert set(actions) == {"platform", *catalog.names}
    assert {action.kind for action in plan.actions} == {ActionKind.CREATE}
    assert fake.events == []


def test_existing_matching_buckets_are_imported(catalog: Catalog, fake: FakeAws) -> None:
    for purpose in PROD_EXISTING:
        fake.buckets.live[bucket_name("prod", purpose)] = matching_live_config(purpose)

    action = _by_target(build_plan(catalog, fake.aws))["prod"]
...
def test_mismatched_existing_bucket_blocks(catalog: Catalog, fake: FakeAws) -> None:
    for purpose in PROD_EXISTING:
        fake.buckets.live[bucket_name("prod", purpose)] = matching_live_config(purpose)
...
def test_bucket_owned_by_another_stack_blocks(catalog: Catalog, fake: FakeAws) -> None:
    name = bucket_name("dev", "avatars")
    fake.buckets.live[name] = matching_live_config("avatars")
    fake.buckets.owners[name] = "rc-dev"

    action = _by_target(build_plan(catalog, fake.aws))["dev"]

    assert action.kind is ActionKind.BLOCKED
    assert f"bucket {name} already belongs to stack rc-dev" in action.details


def test_existing_stacks_update_or_noop(catalog: Catalog, fake: FakeAws) -> None:
    _all_stacks_exist(fake, catalog)
    fake.stacks.previews["rc-env-dev"] = [
        ResourceChange("Modify", "RegionParameter", "AWS::SSM::Parameter")
    ]

    actions = _by_target(build_plan(catalog, fake.aws))
...
def test_busy_or_broken_stack_blocks(catalog: Catalog, fake: FakeAws) -> None:
    fake.stacks.add("rc-env-dev", status="UPDATE_IN_PROGRESS", tags=env_stack_tags("dev"))
    fake.stacks.add(
        "rc-env-staging", status="UPDATE_ROLLBACK_FAILED", tags=env_stack_tags("staging")
    )
...
def test_rolled_back_stack_is_recreated(catalog: Catalog, fake: FakeAws) -> None:
    fake.stacks.add("rc-env-preview3", status="ROLLBACK_COMPLETE", tags=env_stack_tags("preview3"))

    action = _by_target(build_plan(catalog, fake.aws))["preview3"]

    assert action.kind is ActionKind.CREATE
    assert action.replace_failed_stack


def test_removed_environment_is_deleted(catalog: Catalog, fake: FakeAws) -> None:
    _all_stacks_exist(fake, catalog)
...
def test_termination_protected_removed_stack_blocks(catalog: Catalog, fake: FakeAws) -> None:
    add_removed_environment(fake, "preview42")
    stack: Any = fake.stacks.stacks["rc-env-preview42"]
    fake.stacks.add(stack.name, tags=stack.tags, termination_protection=True)

    action = _by_target(build_plan(catalog, fake.aws))["preview42"]

    assert action.kind is ActionKind.BLOCKED
    assert "termination protection" in action.details[0]


def test_unmanaged_core_stack_is_ignored(catalog: Catalog, fake: FakeAws) -> None:
...
def test_blocked_and_deleted_are_listed_first(catalog: Catalog, fake: FakeAws) -> None:
    add_removed_environment(fake, "preview42")
