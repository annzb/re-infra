...
def test_deletes_in_safe_order(catalog: Catalog, fake: FakeAws) -> None:
    add_removed_environment(fake, "preview42")

    teardown("preview42", fake.aws, catalog.names, log=_quiet)
...
def test_protected_environments_are_refused(catalog: Catalog, fake: FakeAws, name: str) -> None:
    stack = fake.stacks.add(f"rc-env-{name}", tags=env_stack_tags(name))
    with pytest.raises(TeardownRefused, match="protected"):
        check_deletable(name, stack, catalog.names - {name})


def test_declared_environment_is_refused(catalog: Catalog, fake: FakeAws) -> None:
    add_removed_environment(fake, "preview3")
    with pytest.raises(TeardownRefused, match="still declared"):
        teardown("preview3", fake.aws, catalog.names, log=_quiet)
    assert fake.events == []


def test_untagged_stack_is_refused(catalog: Catalog, fake: FakeAws) -> None:
    add_removed_environment(fake, "preview42")
...
def test_stack_tagged_for_another_environment_is_refused(catalog: Catalog, fake: FakeAws) -> None:
    add_removed_environment(fake, "preview42")
...
def test_termination_protected_stack_is_refused(catalog: Catalog, fake: FakeAws) -> None:
    add_removed_environment(fake, "preview42")
    fake.stacks.add(
        "rc-env-preview42", tags=env_stack_tags("preview42"), termination_protection=True
    )
...
def test_bucket_outside_environment_prefix_is_refused(catalog: Catalog, fake: FakeAws) -> None:
    add_removed_environment(fake, "preview42")
    fake.stacks.resources["rc-env-preview42"].append(
        StackResource("Stray", bucket_name("prod", "embeddings"), "AWS::S3::Bucket")
    )
...
def test_tables_of_other_environments_are_kept(catalog: Catalog, fake: FakeAws) -> None:
    add_removed_environment(fake, "preview42")
...
def test_retry_after_partial_failure_completes(catalog: Catalog, fake: FakeAws) -> None:
    add_removed_environment(fake, "preview42")
...
def test_missing_stack_is_a_noop(catalog: Catalog, fake: FakeAws) -> None:
    teardown("preview42", fake.aws, catalog.names, log=_quiet)
    assert fake.events == []
